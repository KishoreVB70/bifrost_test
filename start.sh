...
for var in ANTHROPIC_API_KEY OPENAI_API_KEY GEMINI_API_KEY BIFROST_VIRTUAL_KEY; do
  eval val=\$$var
  if [ -z "$val" ]; then
...
# Enable governance via API — MUST succeed or refuse to run
wget -qO- --method=PUT \
  ...
  echo "FATAL: Failed to enable governance — refusing to run without auth" >&2
